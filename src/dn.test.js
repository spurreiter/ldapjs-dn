...
tap.test('constructor', (t) => {
  t.test('throws for non-array', async (t) => {
    t.throws(() => new DN({ rdns: 42 }), Error('rdns must be an array'))
  })

  t.test('throws for non-rdn in array', async (t) => {
    const rdns = [new RDN(), { 'non-string-value': 42 }, new RDN()]
    t.throws(() => new DN({ rdns }))
  })
...
tap.test('childOf', (t) => {
  ...
  t.test('false if differing path', async (t) => {
    ...
    const target = new DN({
      rdns: [new RDN({ dc: 'ldapjs' }), new RDN({ dc: 'com' })],
    })
    t.equal(dn.childOf(target), false)
  })
  ...
  t.test('true if we are a child', async (t) => {
    ...
    const target = new DN({
      rdns: [new RDN({ dc: 'example' }), new RDN({ dc: 'com' })],
    })
...
tap.test('push', (t) => {
  t.test('throws for bad input', async (t) => {
    const dn = new DN()
    t.throws(() => dn.push({ cn: 'foo' }), Error('rdn must be a RDN instance'))
  })

  t.test('adds to the front of the list', async (t) => {
    const dn = new DN({
      rdns: [new RDN({ cn: 'foo' }), new RDN({ dc: 'example' })],
    })
    t.equal(dn.toString(), 'cn=foo,dc=example')
...
tap.test('#fromObject', (t) => {
  t.test('converts an object into an instance', async (t) => {
    const input = {
      cn: 'foo',
      sn: 'bar',
      dc: ['example', 'com'],
    }
    const dn = DN.fromObject(input)
    t.equal(DN.isDn(dn), true)
    t.equal(dn.length, 4)
    t.equal(dn.toString(), 'cn=foo,sn=bar,dc=example,dc=com')
  })

  t.end()
})

tap.test('normalize().toString', { only: true }, (t) => {
  t.test('shall normalize a dn string', async (t) => {
    const input = {
      cn: 'foo',
      sn: 'bar',
      dc: ['example', 'com'],
    ...
    const dn = DN.fromObject(input)
    t.equal(dn.normalize().toString(), 'cn=foo,dc=example,dc=com,sn=bar')
  })
