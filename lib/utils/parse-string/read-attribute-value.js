import { readHexString } from './read-hex-string.js'
import { readEscapeSequence } from './read-escape-sequence.js'
 ...
 */
export function readAttributeValue({ searchBuffer, startPos }) {
  let pos = startPos
    ...
    if (char === 0x22) {
      ...
      if (inQuotes === true) {
        ...
        while (pos < searchBuffer.byteLength) {
          ...
          if (nextChar !== 0x20) {
            throw Error(
              'significant rdn character found outside of quotes at position ' +
                pos
            )
          }
          pos += 1
