...
tap.test('throws for non-string input', async (t) => {
  const input = ['cn=foo']
  t.throws(() => parseString(input), 'input must be a string')
})
...
tap.test('parses basic multiple rdns', async (t) => {
  ...
  t.same(result, [{ dc: 'example' }, { dc: 'com' }])
...
tap.test('throws for invalid attribute type name', async (t) => {
  let input = '3foo=bar'
  t.throws(() => parseString(input), 'invalid attribute type name: 3foo')
  ...
  input = 'føø=bar'
  t.throws(() => parseString(input), 'invalid attribute type name: føø')
})
...
tap.test('rfc 4514 §4 examples', async (t) => {
  const tests = [
    ...
    },
    {
      ...
      expected: [
        {
          '1.3.6.1.4.1.1466.0': new BerReader(
            Buffer.from([0x04, 0x02, 0x48, 0x69])
          ),
        },
      ],
    },
    ...
    if (test.checkBuffer) {
      for (const [i, rdn] of test.expected.entries()) {
        for (const key of Object.keys(rdn)) {
          t.equal(rdn[key].buffer.compare(result[i][key].buffer), 0)
        }
      }
    } else {
