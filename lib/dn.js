...
import { parseString } from './utils/parse-string/index.js'
...
export class DN {
  ...
  /**
   * @param {{[key: string]: string|string[]}} dnObject
   * @returns {DN}
   */
  static fromObject(dnObject) {
    const rdns = []
    for (const [key, value] of Object.entries(dnObject)) {
      if (Array.isArray(value)) {
        for (const item of value) {
          rdns.push(new RDN({ [key]: item }))
        }
      } else {
        rdns.push(new RDN({ [key]: value }))
      }
    }
    return new DN({ rdns })
  }
  ...
  /**
   * normalize the dn into a sorted string to allow usage as a unique identifier
   * @param {(a, b) => number} [keySorter]
   * @return {this}
   */
  normalize(keySorter) {
    const all = new Set()
    const rdns = this.#rdns

    for (const rdn of rdns) {
      ;[...rdn.keys()].forEach((item) => all.add(item))
    }

    const sorted = [...all].sort(keySorter)

    this.#rdns = rdns.sort((a, b) => {
      const aKey = [...a.keys()].sort()[0]
      const bKey = [...b.keys()].sort()[0]
      return sorted.indexOf(aKey) - sorted.indexOf(bKey)
    })
